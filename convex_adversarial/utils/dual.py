    ...
    def g(self, c):
        ...
        for i in range(self.k-2,-1,-1):
            ...
            if i > 0:
                ...
                ###
                mask = self.I_neg[i-1].unsqueeze(1).bool()  # Shape: [50, 1, 100]
                mask = mask.repeat(1, 10, 1)  # Expand along dim 1 (to match the 10 in [50, 10, 100])
                out[mask] = 0
                ###
                # out[self.I_neg[i-1].unsqueeze(1)] = 0
                
                mask_mod = self.I[i-1].unsqueeze(1).bool()
                mask_mod = mask_mod.repeat(1, 10, 1)
                if not self.I_empty[i-1]:
                    if self.alpha_grad: 
                        out[mask_mod] = (self.s[i-1].unsqueeze(1).expand(*nu[i].size())[mask_mod] * 
                                                               nu[i][mask_mod])
                    else:
                        out[mask_mod] = ((self.s[i-1].unsqueeze(1).expand(*nu[i].size())[mask_mod] * 
                                                                                   torch.clamp(nu[i], min=0)[mask_mod])
                                                         + (self.s[i-1].detach().unsqueeze(1).expand(*nu[i].size())[mask_mod] * 
                                                                                   torch.clamp(nu[i], max=0)[mask_mod]))
                nu[i] = out
