...
class NeuralNetParams(object):
  """NeuralNetParams is a class that interfaces the verification code with the neural net parameters (weights)"""
  ...
  def __init__(self, net_weights, net_biases, net_layer_types):
    ...
    self.final_weights = tf.convert_to_tensor(net_weights[self.num_hidden_layers], dtype=tf.float32)
    self.final_bias = tf.convert_to_tensor(net_biases[self.num_hidden_layers], dtype=tf.float32)
