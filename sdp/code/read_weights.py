...
def read_weights(checkpoint, model_json, input_shape=None):
  ...
  Args:
    ...
    model_json: path of json file with model description of the network list of dictionary items for each layer containing 'type',
    'weight_var', 'bias_var' and 'is_transpose' 'type'is one of {'ff', 'ff_relu'}; 'weight_var' is the name of tf variable for weights of layer i;
    'bias_var' is the name of tf variable for bias of layer i; 'is_transpose' is set to True if the weights have to be transposed as
     per convention Note that last layer is always feedforward (verification operates at the layer below the final softmax for more numerical stability)
  Returns:
    net_weights:
    net_biases:
  ...
  net_weights = np.array(net_weights)
